    ...
    if (DesktopPlatform)
    {
        ...
        if (UserFiles.Num() > 0)
        {
            ...
            if (FFileHelper::LoadFileToString(FileContent, *SelectedFile))
            {
                ...
                // Checking for unsupported SVG elements
                ...
                for (FXmlNode* Node : ChildNodes)
                {
                    ...
                    {
                        FString WarningMsg = FString::Printf(TEXT("Unsupported SVG element found: %s"), *NodeTag);
                        UE_LOG(LogTemp, Warning, TEXT("%s"), *WarningMsg);
                        FilePathTextBox->SetText(FText::FromString(WarningMsg));
                        return FReply::Handled();
                    }
                }

                UE_LOG(LogTemp, Log, TEXT("SVG file contains supported elements only."));
